...
export const FishboneCanvas: React.FC<FishboneCanvasProps> = ({
  ...
  return (
    <div className="w-full bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden select-none">
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-32 bg-cyan-600/10 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="overflow-x-auto pb-4 pt-2">
        <div className="min-w-[900px] flex items-center justify-between relative px-6 py-12">
          {/* Central Horizontal Spine (Xương Sống Chính) */}
            ...
            {project.levels.map((lvl, idx) => {
              ...
              return (
                <div key={lvl.id} className="flex flex-col items-center relative group">
                  {/* Top Branch (Small Bone: Process / People) */}
                  ...
                  {/* Bottom Branch (Small Bone: Quality / Finance) */}
          ...
          {/* Fish Tail (Đuôi Cá - Target Mature State) */}
          <div className="relative z-10 flex flex-col items-center shrink-0 ml-4">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-purple-600 to-pink-600 border-2 border-purple-300 text-white flex items-center justify-center text-2xl shadow-xl shadow-purple-600/40">
              🎯
            </div>
            <span className="text-[10px] font-extrabold font-mono text-purple-300 mt-2 uppercase tracking-wider">
              TARGET STATE (L{project.targetLevelNumber})
            </span>
          </div>
        </div>
      </div>
