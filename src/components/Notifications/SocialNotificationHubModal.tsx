...
export const SocialNotificationHubModal: React.FC<SocialNotificationHubModalProps> = ({
  ...
  const handleAddTestNotification = () => {
    audioService.playBeep('decision');
    const platforms: Array<'facebook' | 'instagram' | 'zalo'> = ['facebook', 'instagram', 'zalo'];
    const samplePlat = platforms[Math.floor(Math.random() * platforms.length)];
    const sampleSenders = {
      facebook: 'Nguyễn Văn A (Bạn thân)',
      instagram: 'lan_nguyen.2003',
      zalo: 'Hoàng Anh (Công việc)'
    };
    const sampleMsgs = {
      facebook: 'Vừa nhắc đến bạn trong một bình luận: "Cuối tuần này đi cafe nhé!"',
      instagram: 'Đã gửi cho bạn một tin nhắn thoại & bài viết mới.',
      zalo: 'Sếp gửi file tài liệu dự án và tin nhắn mới cần duyệt gấp.'
    };

    notificationService.addNotification({
      platform: samplePlat,
      title: `${samplePlat.toUpperCase()}: ${sampleSenders[samplePlat]}`,
      message: sampleMsgs[samplePlat],
      type: 'message'
    });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[88vh]">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Tự động gom tin nhắn, tương tác và bài viết mới từ Facebook, Instagram, Zalo
              </p>
            </div>
          </div>
        ...
        <div className="px-4 py-2 border-b border-slate-800/80 bg-slate-950/70 flex items-center justify-between text-xs shrink-0 gap-2">
          <div className="flex items-center gap-3">
            <span className="text-[11px] font-bold text-slate-400 flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
              <span>Trạng thái quét ngầm:</span>
            </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 text-[11px] text-pink-300 font-medium">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                Instagram
              </span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isScanning}
              className="flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] font-semibold transition"
              title="Quét lại toàn bộ tab"
            >
              <RefreshCw className={`w-3 h-3 text-cyan-400 ${isScanning ? 'animate-spin' : ''}`} />
              <span>{isScanning ? 'Đang quét...' : 'Quét Ngay'}</span>
            </button>

            <button
              onClick={handleAddTestNotification}
              className="flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-indigo-600/30 hover:bg-indigo-600 border border-indigo-500/40 text-indigo-200 hover:text-white text-[11px] font-bold transition shadow-sm"
              title="Tạo 1 thông báo mẫu để kiểm tra tính năng"
            >
              <Zap className="w-3 h-3 text-amber-400" />
              <span>Bắn Thử Thông Báo</span>
            </button>
          </div>
        </div>
          ...
          {filteredNotifications.length === 0 ? (
            <div className="py-10 flex flex-col items-center justify-center text-slate-500 space-y-2.5 text-center px-4">
              ...
              <h4 className="text-sm font-bold text-slate-300">
                {selectedFilter === 'all'
                  ? 'Hiện tại chưa có tin nhắn hay tương tác mới nào'
        ...
        {/* Explanatory Footer */}
        <div className="p-3 border-t border-slate-800 bg-slate-950 text-[11px] text-slate-400 flex items-center justify-between px-6">
          <span className="flex items-center gap-1.5">
            <Info className="w-3.5 h-3.5 text-cyan-400" />
            <span>Hệ thống quét 3 tầng: Web Push Notification + Tiêu đề tab `(N)` + Chấm đỏ tin nhắn DOM.</span>
          </span>
          <button
            onClick={onClose}
